...
def article_one(request, id):
    ...
    try:
        a = KbsbArticle.objects.get(id=id)
    ...
    if request.method == 'GET':
        ...
        for al in KbsbArticleLocale.objects.filter(article=a):
            art[al.locale] = {
                'content': al.content,
                'intro': al.intro,
                'title': al.title,
    ...
    if request.method == 'PUT':
        ...
        for l in ('en', 'fr', 'de', 'nl'):
            if l in data:
                try:
                    al = KbsbArticleLocale.objects.get(article=a, locale=l)
                except KbsbArticleLocale.DoesNotExist:
                    al = KbsbArticleLocale(article=a, locale=l)
                lf = data[l]
